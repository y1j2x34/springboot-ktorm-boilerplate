...
CREATE TABLE IF NOT EXISTS `spring-boot-kt`.`oauth2_provider` (
    ...
    `client_authentication_method` VARCHAR(50) NOT NULL DEFAULT 'client_secret_basic' COMMENT 'OAuth2 客户端认证方式',
    `authorization_grant_type` VARCHAR(50) NOT NULL DEFAULT 'authorization_code' COMMENT 'OAuth2 授权类型',
    `authorization_request_params` VARCHAR(2000) NULL COMMENT '授权请求附加参数（JSON）',
    `id_token_jws_algorithm` VARCHAR(20) NULL COMMENT 'ID Token JWS 算法',
...
INSERT INTO `spring-boot-kt`.`oauth2_provider` (
  ...
  'client_secret_basic',
  'authorization_code',
  '{"prompt":"consent","resource":"http://localhost:3001/api/test"}',
  'ES384',
  1,
  0,
  'Logto OIDC provider',
